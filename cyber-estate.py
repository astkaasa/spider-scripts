...
for x in arr:
    ...
    proc = subprocess.Popen(f"cat {site}_{x[0]}_{x[1]}.log | grep -Eo '(http|https)://www.c-estate.com/bookimg/{site}[a-zA-Z0-9&;./?=_-]*' | sort -u", stdout=subprocess.PIPE, shell=True)
    (out, err) = proc.communicate()
    images = []
