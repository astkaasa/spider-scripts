...
for x in arr:
    ...
    item["date"] = today
    item["site"] = f"cyber-estate_{site}_{x[0]}_{x[1]}"
    ...
    # '交通': 'ＪＲ京浜東北・根岸線 川口'
    item["line"] = details['交通'].split(r'\s+')[0]
    item["station"] = details['交通'].split(r'\s+')[1].split('／')[0]
    item["duration"] = details['交通'].split(r'\s+')[1].split('／')[1] if len(details['交通'].split(r'\s+')[1].split('／')) > 1 else ''
    item["images"] = images
    ...
    for image_name, image_url in images.items():
        os.system(f"wget -O '{image_name}.jpg' '{image_url}'")

    data.append(item)
    os.chdir(path)

with open(f"/home/ubuntu/{today}/cyber-estate/{site}_data.json", "w") as f:
